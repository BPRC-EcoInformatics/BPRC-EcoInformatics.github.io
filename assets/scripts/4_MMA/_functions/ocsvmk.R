ocsvmk <- function(sp.name, color.choice, nb=10, save.fig, file.out,do.kmeans=T,thresh=0,prev=50,bg,nb.var) {
...
#setwd(paste("R:\\Intelligent systems for biosecurity\\INVASIVE_SPP\\_OCSVM_results\\",species.name,sep=""))
	setwd(paste(main_wd,sp.name,"_OCSVM_results",bg,nb.var,sep="/"))
	assign(paste("vote",i,sep=""),read.table("votes.txt",header=T))
	assign(paste("model.in.",i,sep=""),read.table(paste("model_in_",bg,nb.var,".txt",sep=''),header=T))
	assign(paste("model.out.",i,sep=""),read.table(paste("model_out_",bg,nb.var,".txt",sep=''),header=T))
...
# --------- #
	...
	setwd(paste(main_wd,sp.name,"_OCSVM_results",bg,nb.var,sep="/"))   #UT CHANGE sep="/"
...
colnames(res) <- c("Vote","%")
write.table(res, paste(sp.name,"_final_votes_",bg,nb.var,".txt",sep=""), row.names=F)
...
# DATA
#setwd(paste(main_wd,species.name,"\\_dist_data",sep=""))
...
write.table(model.in, paste(sp.name,"_models_in_",bg,nb.var,".txt",sep=""), row.names=F)
write.table(model.in, paste(sp.name,"_models_in_",bg,nb.var,".txt",sep=""), row.names=F)  
write.table(model.in, paste(sp.name,"_models_in_",bg,nb.var,".txt",sep=""), row.names=F)  
write.table(model.out,paste(sp.name,"_models_out_",bg,nb.var,".txt",sep=""), row.names=F)  
...
if(save.fig) savePlot(paste(sp.name,"_world_ensemble_oneSVM_",color.choice,"_",bg,nb.var,"_no_pres.png",sep=""), type="png")

# -------------------------------- #
# Plot presences on top of ocsvm
# -------------------------------- #
points(species$xcoord, species$ycoord, col="magenta",pch=20, cex=0.5)
if(save.fig) savePlot(paste(sp.name,"_world_ensemble_oneSVM_",color.choice,"_",bg,nb.var,"_pres.png",sep=""), type="png")
...
points(species$xcoord, species$ycoord, col="magenta",pch=20, cex=0.5)
if(save.fig) savePlot(paste(sp.name,"_World_presences_",bg,nb.var,".png",sep=""), type="png")
...
dat.abs<-na.omit(dat.abs) # I added thisto remove NA rows,giving error on this function, temporary solution, senait.

#if(sp.name=="ks") dat.abs <- dat.abs[1:450000,]
colnames(dat.abs)[1:4] <- c("xcoord", "ycoord","Vote","Prob")
if(file.out) write.table(dat.abs, paste(sp.name,"_data_absences_thresh_",thresh,"_",bg,nb.var,".txt",sep=""), row.names=F)             
save(dat.abs, file=paste(sp.name,"_check_pot_absences_",thresh,"_prev_",prev,"_",bg,nb.var,".RData",sep=""))                        
...
# ----------------------------------------- #
	...
	if(save.fig) savePlot(paste(sp.name,"_World_random_absences_thresh_",thresh,"_prev_",prev,"_",bg,nb.var,".png",sep=""), type="png")
	if(file.out) write.table(dat.abs.sample, paste(sp.name,"_random_abs_thresh_",thresh,"_prev_",prev,"_",bg,nb.var,".txt",sep=""), row.names=F)
	graphics.off()
...
if (do.kmeans) {
	...
	if(file.out) {
		write.table(data.pres.abs, paste(sp.name,"_data_pres_abs_thresh_",thresh,"_prev_",prev,"_",bg,nb.var,".txt",sep=""), row.names=F)
		save(data.abs.kmeans, file=paste(sp.name,"_kmeans_undersampling_object_thresh_",thresh,"_prev_",prev,"_",bg,nb.var,".RData",sep=""))
	}
