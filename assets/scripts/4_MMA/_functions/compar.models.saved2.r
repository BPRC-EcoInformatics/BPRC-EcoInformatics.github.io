...
compar.models.saved2 <- function(x, 
                          y,
                          nrep=200, 
	...
	for(n in 1:nrep){
		...
		for(i in 1:nb.meth){
			...
			print(paste("loop method -",methods[i])) 
			...
			for(j in 1:length(cv.groups)){
				...
				if(is.null(bagging) & is.null(boosting)){
					...
					assign(n.res, train.and.test.model(
							     ...
							     Wts=Wts,
								 beta=beta,test_coord=test_coord,sp=sp,coord.spind_order=coord.spind_order[cv.groups[[j]],]))}
					else{
					assign(n.res, train.and.test.model(
							     train=train,
							     method=methods[i],
...
cat("\n\n")
	...
	for(i in 1:nb.meth){
		print(paste("Arrange result ",methods[i]))
