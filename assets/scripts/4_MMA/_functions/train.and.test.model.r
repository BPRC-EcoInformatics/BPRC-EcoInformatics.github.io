...
# Updated :   May 2009
# Edited: July 2012, by TI, to handle different betas for f-score, default is 1
...
train.and.test.model <- function(train,
                                 ...
                                 priors=NULL,
                                 prob=T,
						   beta=1,test_coord=NULL,sp=FALSE,coord.spind_order=NULL){
	...
	if(!is.null(test)){
		...
		if(ncol(train)==ncol(test)){
			...
			indexes.test <- compute.indexes(conf.test,beta=beta)
			error.test <- 1 - indexes.test[1,1]
		

			if(sp){
			indexes.test.spcorrected <- compute.indexes.spcorrected(prob.test= prob.test,test_coord=test_coord,coord.spind_order=coord.spind_order)
			}
		}
	}
	...
	if(!is.null(test2)){
		...
		if(ncol(train)==ncol(test2)){
			...
			indexes.test2 <- compute.indexes(conf.test2,beta=beta)
	...
	if(!is.null(test3)){
		...
		if(ncol(train)==ncol(test3)){
			...
			indexes.test3 <- compute.indexes(conf.test3,beta=beta)
	...
	if(!is.null(test)){
		...
		if(ncol(train)==ncol(test)){
			...
			if(sp) output$indexes.test.spcorrected  <- indexes.test.spcorrected 
		}
	}
	if(!is.null(test2)){
